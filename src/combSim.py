...
def combine_sim(pathSim_file_name, structSim_file_name, index):
    ...
    d_pair_pathsim_dict = defaultdict(float)
    # d_pair_pathsim_dict = load_from_disk(pathSim_file_name)  # 读取根据疾病元路径计算出来的疾病相似度（字典）
    d_pair_pathsim_dict.update(load_from_disk(pathSim_file_name))   # 读取根据疾病元路径计算出来的疾病相似度（字典）
    ...
    for key in list(d_pair_structsim_dict.keys()):  # 从结构相似字典依次取出一个记录(这个字典应该是存储了任意两点之间的结构)
        ...
        if d_pair_pathsim_dict[key] != 0:
            pathsim_score = d_pair_pathsim_dict[key]
        elif d_pair_pathsim_dict[tuple(reversed(key))] != 0:
            pathsim_score = d_pair_pathsim_dict[tuple(reversed(key))]

        # if key in list(d_pair_pathsim_dict.keys()):  # 字典只保存了相似度值大于0的记录
        #     pathsim_score = d_pair_pathsim_dict[key]  # 语义相似度值>0，获取该相似度值
        #     if (key[0] == 'D12930') & (key[1] == 'D5844'):
        #         print('pathsim_score: ', pathsim_score)
        # elif key in list(reversed(list(d_pair_pathsim_dict.keys()))):
        #     pathsim_score = d_pair_pathsim_dict[key]
        #     if (key[0] == 'D5844') & (key[1] == 'D12930'):
        #         print('pathsim_score: ', pathsim_score)
